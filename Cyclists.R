...
library("anytime")
getwd()
setwd("D:/GOOGLE(DA)Coursera/Capstone Coursera DA/Case Study/Case Study 1/Data Analysis using R")
...
all_trips <- all_trips %>% 
  select(-c(start_lat,start_lng,end_lat,end_lng,gender,"X01...Rental.Details.Duration.In.Seconds.Uncapped",Member.Gender,tripduration,"X05...Member.Details.Member.Birthday.Year"))

all_trips <- all_trips %>% 
  select(-c(birthyear))


#STEP 3: CLEANING UP DATA AND AND ADD DATA TO PREPARE FOR ANALYSIS
